...
import { Luranta, TransportError } from "../dist/index.js"
...
void test("the built SDK runs with Node Fetch", async () => {
  ...
  const page = await luranta.reference.roles.list()
...
void test("Node network failures surface without a retry", async () => {
  ...
  await assert.rejects(luranta.reference.roles.list(), TransportError)
